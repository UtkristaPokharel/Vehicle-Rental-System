...
export default function HostDashboard() {
  ...
  useEffect(() => {
    ...
    // Show loading state immediately when starting data fetch
    setLoading(true);
    fetchHostData();
  }, [navigate]);
  ...
  const fetchHostData = useCallback(async () => {
    ...
    try {
      ...
      const [vehiclesResponse, bookingsResponse] = await Promise.allSettled([
        fetch(getApiUrl(`api/user/user/${userId}`), {
          headers: { Authorization: `Bearer ${token}` }
        }),
        fetch(getApiUrl(`api/bookings/host/${userId}`), {
          headers: { Authorization: `Bearer ${token}` }
        })
      ]);
      ...
      if (vehiclesResponse.status === 'fulfilled' && vehiclesResponse.value.ok) {
        ...
        setAnalytics(prev => ({
          ...
          activeVehicles: vehiclesList.filter(v => v.isActive).length,
          availableVehicles: vehiclesList.filter(v => v.isActive && v.isAvailable !== false).length
        }));
      } else {
      ...
      if (bookingsResponse.status === 'fulfilled' && bookingsResponse.value.ok) {
        ...
        // Calculate booking analytics efficiently
        const analytics = bookingsList.reduce((acc, booking) => {
          ...
          return acc;
        }, { totalBookings: 0, completedBookings: 0, pendingBookings: 0, totalRevenue: 0 });
  ...
  }, []); // Empty dependency array since we're using localStorage values

  const handleVehicleAvailability = useCallback(async (vehicleId, currentAvailability) => {
    const token = localStorage.getItem('token') || localStorage.getItem('adminToken');
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Update state optimistically without full refetch for better UX
        ...
        setAnalytics(prev => ({
          ...prev,
          availableVehicles: prev.availableVehicles + (!currentAvailability ? 1 : -1)
        }));
      } else {
  ...
  const filteredBookings = useMemo(() => {
    return bookings.slice(0, 10); // Show only recent 10 bookings for better performance
  }, [bookings]);
          ...
          <div className="bg-white rounded-xl shadow-sm">
            <div className="border-b border-gray-200">
              <div className="flex space-x-8 px-6">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className="h-4 bg-gray-200 rounded w-16 my-4 animate-pulse"></div>
                ))}
              </div>
            </div>
            <div className="p-6">
